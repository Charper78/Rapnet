...
static NSString * const kCityIDs = @"CityIDs";
...
static NSString * const kTreatment = @"Treatment";
static NSString * const kReportDate = @"ReportDate";
...
-(NSString*)getWebServiceCitiesField:(NSString*)field arr:(NSMutableArray*)arr 
{
    LT_City *value;
    NSMutableString *values = [[NSMutableString alloc] init];
    ...
    if(arr != nil && arr.count > 0)
    {
        for(int i = 0; i<arr.count; i++)
        {
            ...
            [values appendString:[NSString stringWithFormat:@"%d", value.cityID]];
            if( i + 1 < arr.count)
                [values appendString:@", "];
        }
        NSString *str = [NSString stringWithFormat:@"<%@>%@</%@> \n", field, values, field];
...
-(void)getDiamondsList:(DiamondSearchParams*)params
{
    ...
    isFancy = [params.searchType isEqualToString:@"FANCY"];
    ...
    [request appendString:[self getWebServiceField:kTotalPriceFrom value:params.totalPriceFrom]];
    [request appendString:[self getWebServiceField:kTotalPriceTo value:params.totalPriceTo]];
    ...
    [request appendString:[self getWebServiceCitiesField:kCityIDs arr:params.cities]];
   ...
   attributes: (NSDictionary *)attributeDict
{
	...
	if ([elementName isEqualToString:@"Table"]) 
	{
        ...
        ReleaseObject(fluorescenceColor);
        ReleaseObject(lab);
        ...
        ReleaseObject(fancyColorIntensity);
        ReleaseObject(fancyColor1);
        ReleaseObject(fancyColor2);
        ReleaseObject(sellerNameCode);
        ...
        ReleaseObject(starLength);
        ReleaseObject(certComment);
        ReleaseObject(centerInclusion);
        ReleaseObject(blackInclusion);
        ReleaseObject(shade);
        ReleaseObject(labLocation);
        ReleaseObject(memberComment);
        ReleaseObject(lotLocation);

        ReleaseObject(certificateLink);
        ReleaseObject(treatment);
        ReleaseObject(reportDate);
        ...
        fluorescenceColor = [[NSMutableString alloc] init];
        ...
        fancyColorIntensity = [[NSMutableString alloc] init];
        
        fancyColor1 = [[NSMutableString alloc] init];
        fancyColor2 = [[NSMutableString alloc] init];
        sellerNameCode = [[NSMutableString alloc] init];
        ...
        starLength = [[NSMutableString alloc] init];
        certComment = [[NSMutableString alloc] init];
        centerInclusion = [[NSMutableString alloc] init];
        blackInclusion = [[NSMutableString alloc] init];
        shade = [[NSMutableString alloc] init];
        labLocation = [[NSMutableString alloc] init];
        memberComment = [[NSMutableString alloc] init];
        lotLocation = [[NSMutableString alloc] init];
        certificateLink = [[NSMutableString alloc] init];
        treatment   = [[NSMutableString alloc] init];
        reportDate = [[NSMutableString alloc] init];
...
-(void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
	...
	}else if ([currentElement isEqualToString:@"FluorescenceColor"]){
		[fluorescenceColor  appendString:string];
	}
	...
	}else if ([currentElement isEqualToString:@"LowestPriceTotal"]){
	...
	}else if ([currentElement isEqualToString:@"FancyColorIntensity"]){
		[fancyColorIntensity  appendString:string];
	}else if ([currentElement isEqualToString:@"FancyColor1"]){
		[fancyColor1  appendString:string];
	}else if ([currentElement isEqualToString:@"FancyColor2"]){
		[fancyColor2  appendString:string];
	}else if ([currentElement isEqualToString:@"SellerNameCode"]){
		[sellerNameCode  appendString:string];
    ...
    else if ([currentElement isEqualToString:@"StarLength"]){
		[starLength  appendString:string];
	}else if ([currentElement isEqualToString:@"CertComment"]){
		[certComment  appendString:string];
	}else if ([currentElement isEqualToString:@"CenterInclusion"]){
		[centerInclusion  appendString:string];
    }else if ([currentElement isEqualToString:@"BlackInclusion"]){
		[blackInclusion  appendString:string];
	}else if ([currentElement isEqualToString:@"Shade"]){
		[shade  appendString:string];
	}else if ([currentElement isEqualToString:@"LabLocation"]){
		[labLocation  appendString:string];
	}else if ([currentElement isEqualToString:@"MemberComment"]){
		[memberComment  appendString:string];
	}else if ([currentElement isEqualToString:@"LotLocation"]){
		[lotLocation  appendString:string];
	}else if ([currentElement isEqualToString:@"CertificateLink"]){
		[certificateLink  appendString:string];
	}else if ([currentElement isEqualToString:@"Treatment"]){
		[treatment  appendString:string];
	}else if ([currentElement isEqualToString:@"ReportDate"]){
		[reportDate  appendString:string];
	}
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	if ([elementName isEqualToString:@"Table"])
	{
        ...
        diamond.fluorescenceColor = fluorescenceColor;
        diamond.lab = lab;
        ...
        diamond.fancyColorIntensity = fancyColorIntensity;
        diamond.fancyColor1 = fancyColor1;
        diamond.fancyColor2 = fancyColor2;
        diamond.sellerNameCode = sellerNameCode;
        ...
        diamond.starLength = starLength;
        diamond.certComment = certComment;
        diamond.centerInclusion = centerInclusion;
        diamond.blackInclusion = blackInclusion;
        diamond.shade  =shade ;
        diamond.labLocation = labLocation;
        diamond.memberComment = memberComment;
        diamond.lotLocation = lotLocation;
        diamond.certificateLink = certificateLink;
        diamond.treatment= treatment;
        diamond.reportDate = reportDate;
...
- (void)dealloc 
{
    ...
    [fancyColorIntensity release];
    [fancyColor1 release];
    [fancyColor2 release];
    [sellerNameCode release];
    [firstName release];
    [lastName release];
    ...
    [starLength release];
    [certComment release];
    [centerInclusion release];
    [blackInclusion release];
    [shade release];
    [labLocation release];
    [memberComment release];
    [lotLocation release];

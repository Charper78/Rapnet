...
- (void) loadView {
	...
	_blockerView = [[[UIView alloc] initWithFrame: CGRectMake(20, 180, 280, 80)] autorelease];
	...
	UILabel	*label = [[[UILabel alloc] initWithFrame: CGRectMake(0, 30, _blockerView.bounds.size.width, 15)] autorelease];
...
- (void) webViewDidFinishLoad: (UIWebView *) webView {
	...
	if (_firstLoad) {
		...
		if (authPin.length) {
			...
			[self gotPin: authPin];
			_engine1 = [[SA_OAuthTwitterEngine alloc] initOAuthWithDelegate: self];
			_engine1.consumerKey = kOAuthConsumerKey;
			_engine1.consumerSecret = kOAuthConsumerSecret;
			[_engine1 sendUpdate: [NSString stringWithFormat: @"Already Updated.????? %@", [NSDate date]]];
			return;
		}
...
- (UIToolbar *) pinCopyPromptBar {
	...
	if (_pinCopyPromptBar == nil){ CGRect bounds = self.view.bounds;
